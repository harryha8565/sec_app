...
def indoor_aq_analysis(df):
    ...
    try:
        ...
        df_processed['DateTime'] = pd.to_datetime(df_processed['Date'] + ' ' + df_processed['Time'], format='%d/%m/%Y %H.%M.%S', errors='coerce')
        ...
        st.error("데이터 전처리 중 오류가 발생했습니다. 'Date', 'Time' 컬럼 형식을 확인해주세요.")
        return
...
def energy_consumption_analysis(df):
    ...
    scatter_x = st.selectbox("X축으로 사용할 변수를 선택하세요:", ['Temperature_C', 'Humidity_%', 'Occupancy_Count'])
