...
if df_integrated is not None:
    ...
    if menu == "📋 데이터 개요":
        ...
        st.markdown(f"선택한 변수 **{col}** 의 히스토그램입니다. (필터 적용)")
        fig = px.histogram(filtered, x=col, nbins=30)  # 수정된 부분
        st.plotly_chart(fig, use_container_width=True)

        st.markdown("선택한 값 범위에 포함된 데이터입니다.")
        st.write(f"총 {len(filtered)}개 데이터가 선택되었습니다.")
